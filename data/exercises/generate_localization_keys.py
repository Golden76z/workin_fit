...
for exercise in all_exercises:
    ...
    arb_fr_entries[name_key] = exercise['name']  # Will need translation
    
    # Description
    desc_key = f'exercise_{ex_id}_description'
    arb_en_entries[desc_key] = exercise['description']
    arb_fr_entries[desc_key] = exercise['description']  # Will need translation
    ...
    if exercise.get('beginnerTips'):
        ...
        arb_fr_entries[tips_key] = exercise['beginnerTips']  # Will need translation
...
with open(fr_arb_path, 'w', encoding='utf-8') as f:
    ...
    print(f'✅ Updated {fr_arb_path} with {len(arb_fr_entries)} exercise entries (needs translation)')
...
print('⚠️  Note: French translations need to be added manually to app_fr.arb')
